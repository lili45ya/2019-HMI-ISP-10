...
def model(x_train, y_train, x_test, y_test, learning_rate=0.01):
    ...
    X_place = tf.placeholder(tf.float32, [None, 400])
    ...
    dist = tf.reduce_sum(tf.square(X_place), 1)
    dist = tf.reshape(dist, [-1, 1])
    sq_dists = tf.add(tf.subtract(dist, tf.multiply(2., tf.matmul(X_place, tf.transpose(X_place)))), tf.transpose(dist))
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(1000):
            ...
            dist = tf.reshape(dist, [-1, 1])
            sq_dists = tf.add(tf.subtract(dist, tf.multiply(2., tf.matmul(tf.transpose(x_trainfloat), x_trainfloat))),
                              tf.transpose(dist))
            my_kernel = tf.exp(tf.multiply(gamma, sq_dists))
...
if __name__ == "__main__":
    ...
    parser.add_argument("train_val_split", help="train_val_split", type=float)
    parser.add_argument("--wav_dir", help="wav_dir", required=True, type=str)
    args = parser.parse_args()
    data = preprocess_data(args.wav_dir, 1)
    data = random.shuffle(data)
    data_x = data[:, :-1]
    data_y = data[:, -1]
    num = np.shape(data)[0]
    train_x = data_x[:int(round(-args.train_val_split*num))]
    val_x = data_x[int(round(-args.train_val_split*num)):]
    train_y = data_y[:int(round(-args.train_val_split*num))]
    val_y = data_x[int(round(-args.train_val_split*num)):]
    train_loss = model(train_x, train_y, val_x, val_y)
